...
import { eventMemberService } from "../../eventMembers/services/eventMemberService";
...
export function useEventsPage() {
  ...
  const [joinedEventMembers, setJoinedEventMembers] = useState([]);
  ...
  const canLoadTeachers =
    currentRole === "schooladmin" || currentRole === "teacher";
  ...
  const loadJoinedEventMembers = async () => {
    if (!isStudent) {
      setJoinedEventMembers([]);
      return;
    }

    try {
      const result = await eventMemberService.getMyEvents();
      const data = normalizeResultData(result);

      setJoinedEventMembers(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error(error);
      showToast(
        getErrorMessage(
          error,
          "Katıldığınız etkinlikler yüklenirken hata oluştu."
        ),
        "error"
      );
    }
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      await Promise.all([loadEvents(), loadTeachers(), loadJoinedEventMembers()]);
    };
  ...
  const joinedEventIds = useMemo(() => {
    return joinedEventMembers
      .map((member) => member?.eventId || member?.EventId)
      .filter(Boolean)
      .map(String);
  }, [joinedEventMembers]);
  ...
  const joinedEvents = useMemo(() => {
    return events.filter((event) => {
      ...
      return joinedEventIds.includes(String(eventId));
    });
